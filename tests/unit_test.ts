...
const VENDORED_ZSTD = "../netlify/edge-functions/lib/vendor/zstd-wasm.js";

Deno.test("the vendored bundle matches the version in package.json", async () => {
  // It is generated by scripts/vendor-zstd.mjs and committed. Bumping the
  // dependency without regenerating would leave the edge running whichever
  // build happened to be vendored last, with nothing to say so.
  const source = await Deno.readTextFile(new URL(VENDORED_ZSTD, import.meta.url));
  const vendored = source.match(/@bokuweb\/zstd-wasm@([\d.]+)/)?.[1];
  assertTruthy(vendored, "the vendored bundle should record the version it came from");

  const packageJson = JSON.parse(
    await Deno.readTextFile(new URL("../package.json", import.meta.url)),
  );
  const declared = packageJson.dependencies["@bokuweb/zstd-wasm"].replace(/^[\^~]/, "");
  assertEquals(vendored, declared, "run `npm run vendor:zstd` after changing the dependency");
});

Deno.test("the vendored bundle needs no filesystem", async () => {
  // This is the whole reason it exists. Netlify's bundler resolves the package
  // through the `node` export condition, whose init is
  // `readFile(resolve(__dirname, './zstd.wasm'))` - unrunnable on Deno Deploy,
  // and the cause of three deploys' worth of misdiagnosis. The vendored bundle
  // is the browser build with the wasm inlined; if either of these strings
  // reappears, the wrong build has been bundled again.
  const source = await Deno.readTextFile(new URL(VENDORED_ZSTD, import.meta.url));

  assertEquals(source.includes("fs/promises"), false, "the Node build got bundled");
  assertEquals(source.includes("__dirname"), false, "the Node build got bundled");
  // And the binary is inline, so instantiation is CPU rather than a request the
  // edge function would need an absolute origin to make.
  assertStringIncludes(source, "wasmBinaryBase64");
});

Deno.test("compression actually runs through the vendored bundle", async () => {
  // The round-trip test above proves the bytes decode. This proves the size
  // claim on a real page, which is the thing the proposal is selling.
  const { compressWithDictionary } = await import("../netlify/edge-functions/lib/zstd.ts");
  const dictionary = await getShellDictionary();
  const page = new TextEncoder().encode(
    await htmlToString(renderHome(dictionaryStories(), 2, "top", `${ORIGIN}/top/2`)),
  );

  const frame = await compressWithDictionary(page, dictionary.bytes);

  assertEquals(frame.length < page.length / 20, true, `${page.length} -> ${frame.length}`);
});

/** Thirty plausible stories, for size measurements on a realistic page. */
function dictionaryStories() {
  return Array.from({ length: 30 }, (_, i) => ({
    id: 44000000 + i,
    title: `A genuinely typical Hacker News submission title number ${i}`,
    points: 40 + i,
    user: "reader",
    time: 1760000000,
